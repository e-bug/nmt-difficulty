...
def main(parsed_args):
    ...
    if args.log2:
        ...
        print('| Loss: {:.4f}, Perplexity: {:.2f}, Cross-entropy: {:.4f}'.format(avg_nll_loss, np.power(2, avg_nll_loss), avg_nll_loss*(count-len(dataset))/len(dataset)))
    else:
        print('| Loss: {:.4f}, Perplexity: {:.2f}, Cross-entropy: {:.4f}'.format(avg_nll_loss, np.exp(avg_nll_loss), avg_nll_loss*(count-len(dataset))/len(dataset)))
...
def cli_main():
    ...
    parser.add_argument("--log2", action='store_true', help="Fairseq defaults to natural log")
